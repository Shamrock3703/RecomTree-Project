...
To compile the project and generate the JAR file, open your terminal in the project root directory and run: mvn clean package
...
###3. Start the Client###
Open a new terminal window (do not close the server) and run the client to connect to the server: java -cp target/movie-recom-tree-1.0-SNAPSHOT.jar ch.hevs.client.Client

Note: The -cp flag specifies the Classpath, allowing you to select which main class (RecomServer or Client) to execute from the same JAR file.
